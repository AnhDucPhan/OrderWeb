...
const Header = () => {
    ...
    useEffect(() => {
        // Đánh dấu là đã ở trên trình duyệt (Client), không còn ở Server nữa
        setIsMounted(true);
        ...
        handleScroll(); // Tính toán tọa độ ngay lập tức
    ...
    return (
        <>
            <header
                ...
                className={`fixed top-0 left-0 w-full h-[60px] sm:h-[80px] lg:h-[100px] flex items-center justify-between px-4 lg:px-[30px] font-text-header font-dm-sans z-50 transition-all duration-500 ease-out
                ${!isMounted ? "opacity-0 -translate-y-5" : "opacity-100 translate-y-0"}
                    ...
                    <nav aria-label="Điều hướng chính" className="flex justify-center">
                        <ul className={`grid grid-cols-5 gap-1 text-center transition-colors duration-300 ${scrolled ? "text-[#0B0B24]" : "text-white"}`}>
                            ...
                            <li className="header-center relative group">
                                ...
                                <div className="absolute top-full left-1/2 -translate-x-1/4 mt-2 w-64 bg-white shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                                    <ul className="flex flex-col p-4 gap-2 pl-6 text-left">
                                        <li className="color-text-header">Cà Phê Pha Phin</li>
