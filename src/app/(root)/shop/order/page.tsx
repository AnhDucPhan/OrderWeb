...
import { ConfigProvider, Tag, Button, Modal, Divider, Empty } from 'antd';
import { EyeOutlined, ShoppingOutlined, ArrowLeftOutlined } from '@ant-design/icons';
import { useSession } from 'next-auth/react';

// --- MOCK DATA TẠM THỜI ĐỂ HIỂN THỊ UI ---
// (Sau này bạn sẽ thay bằng data lấy từ API GET /orders)
const mockOrders = [
    {
        id: 1,
        orderCode: '1774121305000',
        createdAt: '2026-03-22T10:30:00Z',
        totalAmount: 10500000,
        paymentStatus: 'COMPLETED',
        orderStatus: 'PENDING',
        items: [
            { id: 1, quantity: 1, price: 5500000, product: { name: 'Áo Vest Nam Cao Cấp', thumbnail: '/images/placeholder.png' } },
            { id: 2, quantity: 2, price: 2500000, product: { name: 'Quần Âu Đen Dáng Ôm', thumbnail: '/images/placeholder.png' } }
        ]
    },
    {
        id: 2,
        orderCode: '1774100000000',
        createdAt: '2026-03-15T14:20:00Z',
        totalAmount: 3200000,
        paymentStatus: 'COMPLETED',
        orderStatus: 'DELIVERED',
        items: [
            { id: 3, quantity: 1, price: 3200000, product: { name: 'Giày Da Nam Oxford', thumbnail: '/images/placeholder.png' } }
        ]
    }
];
...
const OrderHistoryPage = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                const response = await fetch('http://localhost:8386/order/history', {
                    headers: {
                ...
                if (response.ok) {
                    ...
                    setOrders(data); // Đổ data thật vào state
                }
            } catch (error) {
    ...
    const renderOrderStatus = (status: string) => {
        switch (status) {
            case 'PENDING': return <Tag color="processing" className="font-[DM_Sans] rounded-full px-3">Chờ xác nhận</Tag>;
            case 'PROCESSING': return <Tag color="warning" className="font-[DM_Sans] rounded-full px-3">Đang đóng gói</Tag>;
            case 'SHIPPED': return <Tag color="purple" className="font-[DM_Sans] rounded-full px-3">Đang giao hàng</Tag>;
            case 'DELIVERED': return <Tag color="success" className="font-[DM_Sans] rounded-full px-3">Đã giao</Tag>;
            case 'CANCELLED': return <Tag color="error" className="font-[DM_Sans] rounded-full px-3">Đã hủy</Tag>;
                    ...
                    <div className="mt-6 font-[DM_Sans]">
                        <div className="grid grid-cols-2 gap-4 mb-6 bg-gray-50 p-4 rounded-lg border border-gray-100">
                            ...
                            <div>
                                ...
                                <p className="font-bold text-[#111111]">{selectedOrder.paymentStatus === 'COMPLETED' ? 'Đã thanh toán (VNPay)' : 'Thanh toán khi nhận hàng'}</p>
                            </div>
                        </div>
